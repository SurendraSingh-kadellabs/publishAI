...
$(document).ready(function () {
   ...
   $(document).on("click", ".btn-datatable-row-action", function (ev) {
        ev.preventDefault();
                    ...
                    if (willDelete) {
                        ...
                        axios({
                            ...
                            .then(function (response) {
                                ...
                                if (response.data.status_output == 1) {
                                    iziToast.success({
    ...
    $.customDesignedBlockUI = function () {
        $.blockUI({
            css: {
                border: 'none',
                padding: '15px',
...
// user Setting And App Setting (Left Side Navigation Bar) Dark Mode Effecting code 
// If Window Width > 1024 then will property add
if ($(window).width() > 1024) {
    $('#toggle-button-double-logo-sidebar_small-large').on("click", function () {
        if ($("body").hasClass("sidebar-mini")) {
            $('.nav-menu-dark1').removeClass('bg-dark text-white');
        } else {
            $('.nav-menu-dark1').addClass('bg-dark text-white');
        }
    });
}
...
function checkBeforeSubmit() {
    $(".submit_btn_cls").attr('disabled', 'disabled');
    $(".waitLoader").addClass("spinner-border spinner-border-sm");
    $(".waitLoader").text("");
    $(".waitLoader1").removeClass("fas fa-save");
}
